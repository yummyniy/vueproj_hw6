    ...
    <div class="modal-dialog modal-xl" role="document">
      <div class="modal-content border-0">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="updatePaid(tempOrder)">修改付款狀態</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    updatePaid (order) {
      ...
      this.$http.put(`${process.env.VUE_APP_API}/api/${process.env.VUE_APP_PATH}/admin/order/${order.id}`, { data: paid })
        .then((res) => {
          this.closeModal()
